...
CandleList_CandlesBetweenTimestamps :: proc(candleList : CandleList, startTimestamp : i32, endTimestamp : i32) -> ([]Candle, i32)
{
	...
	if candleList.timeframe == .MONTH
	{
		...
		candleIndex := startTimestamp / FOUR_YEARS * 48
		remainingTimestamp := startTimestamp % FOUR_YEARS
		...
		candleIndex += remainingIndex

		if candleListLen <= candleIndex
		{
			// Start is further right than the rightmost candle
			return nil, -1
		}

		endCandleIndex := endTimestamp / FOUR_YEARS * 48
		remainingEndTimestamp := endTimestamp % FOUR_YEARS

		remainingIndex = 47

		for remainingEndTimestamp < monthlyIncrements[remainingIndex]
		{
			remainingIndex -= 1
		}

		candleIndex = math.clamp(candleIndex, 0, lastCandleIndex)
		endCandleIndex = math.clamp(endCandleIndex + remainingIndex, 0, lastCandleIndex + 1)

		return candleList.candles[candleIndex:endCandleIndex], candleIndex
	}
...
CandleList_RoundTimestamp :: proc(candleList : CandleList, timestamp : i32) -> i32
{
	return CandleList_IndexToTimestamp(candleList, CandleList_TimestampToIndex(candleList, timestamp))
}

CandleList_RoundTimestamp_Clamped :: proc(candleList : CandleList, timestamp : i32) -> i32
{
	return CandleList_IndexToTimestamp(candleList, CandleList_TimestampToIndex_Clamped(candleList, timestamp))
}
